import { J as BufferGeometry, bn as Float32BufferAttribute, ei as Matrix3, mn as Euler, ri as Mesh, ti as Matrix4, zs as Vector3 } from "./three.module-BLr34Qqw.js";
//#region node_modules/three/examples/jsm/geometries/DecalGeometry.js
/**
...
var DecalGeometry = class extends BufferGeometry {
	...
	*/
	constructor(mesh = new Mesh(), position = new Vector3(), orientation = new Euler(), size = new Vector3(1, 1, 1)) {
		...
		if (normals.length > 0) this.setAttribute("normal", new Float32BufferAttribute(normals, 3));
		function generate() {
			let decalVertices = [];
			...
			if (geometry.index !== null) {
				...
				for (let i = 0; i < index.count; i++) {
					...
					} else pushDecalVertex(decalVertices, vertex);
				}
			} else {
			...
			for (let i = 0; i < decalVertices.length; i++) {
				...
				uvs.push(.5 + decalVertex.position.x / size.x, .5 + decalVertex.position.y / size.y);
				decalVertex.position.applyMatrix4(projectorMatrix);
				vertices.push(decalVertex.position.x, decalVertex.position.y, decalVertex.position.z);
				if (decalVertex.normal !== null) normals.push(decalVertex.normal.x, decalVertex.normal.y, decalVertex.normal.z);
			}
		}
		function pushDecalVertex(decalVertices, vertex, normal = null) {
			...
			} else decalVertices.push(new DecalVertex(vertex.clone()));
		}
		function clipGeometry(inVertices, plane) {
			const outVertices = [];
			const s = .5 * Math.abs(size.dot(plane));
		...
		function clip(v0, v1, p, s) {
			...
			const s0 = d0 / (d0 - (v1.position.dot(p) - s));
			const position = new Vector3(v0.position.x + s0 * (v1.position.x - v0.position.x), v0.position.y + s0 * (v1.position.y - v0.position.y), v0.position.z + s0 * (v1.position.z - v0.position.z));
			let normal = null;
			if (v0.normal !== null && v1.normal !== null) normal = new Vector3(v0.normal.x + s0 * (v1.normal.x - v0.normal.x), v0.normal.y + s0 * (v1.normal.y - v0.normal.y), v0.normal.z + s0 * (v1.normal.z - v0.normal.z));
			return new DecalVertex(position, normal);
		}
	}
};
